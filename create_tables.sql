...
create table Experiment(
	...
	foreign key(strain_id) references Strain(strain_id) on delete no action on update cascade,
	foreign key(media_id) references Media(media_id) on delete no action on update cascade,
	foreign key(source_id) references Source(source_id) on delete no action on update cascade) engine=InnoDB;
...
create table Threshold(
	...
	OD48hr float,
	OD72hr float,
	primary key(strain_id, media_id, run_date),
	foreign key(strain_id) references Strain(strain_id) on delete no action on update cascade,
	foreign key(media_id) references Media(media_id) on delete no action on update cascade) engine=InnoDB;
